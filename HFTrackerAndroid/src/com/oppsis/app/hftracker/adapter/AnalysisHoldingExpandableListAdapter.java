...
public class AnalysisHoldingExpandableListAdapter extends BaseExpandableListAdapter{

	private Context mContext;
	private List<FundAnalysis> mAnalysisHoldingList; 
	private HashMap<String, List<Map.Entry<String, Float>>> mAnalysisHoldingListChild;
	private Map<String, StockQuoteObject> mStockQuote;
	
	public AnalysisHoldingExpandableListAdapter(Context context, List<FundAnalysis> listDataHeader,
			HashMap<String, List<Map.Entry<String, Float>>> listChildData,Map<String, StockQuoteObject> stockData) {
		this.mContext = context;
		this.mAnalysisHoldingList = listDataHeader;
		this.mAnalysisHoldingListChild = listChildData;
	...
	public Object getChild(int groupPosition, int childPosition) {
		return mAnalysisHoldingListChild.get(mAnalysisHoldingList.get(groupPosition).getId()).get(childPosition);
	}
	...
	public View getChildView(int groupPosition, int childPosition,
		...
		if (convertView == null) {
			...
			convertView = LayoutInflater.from(mContext).inflate(R.layout.analysis_holding_expandable_list_item_child, parent,false);
			
			viewHolder.pic = (ImageView)convertView.findViewById(R.id.analysis_holding_item_child_pic);
			viewHolder.port = (TextView)convertView.findViewById(R.id.analysis_holding_item_child_port);
			viewHolder.manager = (TextView)convertView.findViewById(R.id.analysis_holding_item_child_manager);
			viewHolder.fund = (TextView)convertView.findViewById(R.id.analysis_holding_item_child_fund);
	...
	public int getChildrenCount(int groupPosition) {
		return mAnalysisHoldingListChild.get(mAnalysisHoldingList.get(groupPosition).getId()).size();
	}

	@Override
	public Object getGroup(int groupPosition) {
		return mAnalysisHoldingList.get(groupPosition);
	}

	@Override
	public int getGroupCount() {
		return mAnalysisHoldingList.size();
	}
	...
	public View getGroupView(final int groupPosition, boolean isExpanded,
		...
		final FundAnalysis analysisHolding = mAnalysisHoldingList.get(groupPosition);
		HeaderViewHolder holder;
		if(convertView == null){
			holder = new HeaderViewHolder();

			convertView = LayoutInflater.from(mContext).inflate(R.layout.analysis_holding_expandable_list_item, parent,false);
			holder.ticker = (TextView)convertView.findViewById(R.id.analysis_holding_item_ticker);
			holder.security = (TextView)convertView.findViewById(R.id.analysis_holding_item_name);
			...
			holder.arrow = (ImageView)convertView.findViewById(R.id.analysis_holding_item_arrow);
		...
		final String ticker = analysisHolding.getTicker();
		holder.ticker.setText(ticker);
		holder.security.setText(analysisHolding.getSecurity());
		...
		holder.ticker.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(ticker != null && !"".equals(ticker)){
					...
					SecurityDataObject data = new SecurityDataObject(analysisHolding.getTicker(), analysisHolding.getSecurity(), null,SecurityDataObject.ICON_HIDE);
	...
	static class HeaderViewHolder{
		TextView ticker,security,price,changePercent;
		ImageView arrow;
		//ImageButton imgBtn;
	}
